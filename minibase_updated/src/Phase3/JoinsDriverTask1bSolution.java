...
public class JoinsDriverTask1bSolution implements GlobalConst{
	...
	private FldSpec[] Rprojection = new FldSpec[4];
	...
	private CondExpr[] queryConfiguration = new CondExpr[3];
	 ...
	 */
	public JoinsDriverTask1bSolution(String RFilePath, String SFilePath, String QueryFilePath)
			throws FileNotFoundException {
		...
		queryConfiguration[1] = new CondExpr();
		queryConfiguration[2] = new CondExpr();
	...
	public void QueryTask2c() {		
		...
		try {
			nlj = new NestedLoopsJoins(rAttrTypes, 4, null, sAttrTypes, 4, null, 10,
	...
	private void printResult() throws FileNotFoundException{
		File file  = new File("src/tests/Task1b.txt");
	...
	private void loadQeuryExpr(String filePath, CondExpr[] expr)
		...
		if(input.hasNext()){
			...
			expr[1].operand1.symbol = new FldSpec(new RelSpec(RelSpec.outer), Integer.parseInt(eachWhereFiled1));
			expr[1].operand2.symbol = new FldSpec(new RelSpec(RelSpec.innerRel), Integer.parseInt(eachWhereFiled22));
			int op2 = Integer.parseInt(eachWhere2[1]);
			expr[1].op = new AttrOperator(mapOpNum(op2));
		}else{
