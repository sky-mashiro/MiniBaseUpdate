...
public class JoinsDriverTask2dSolutiona implements GlobalConst{

	private boolean OK = true;
	private boolean FAIL = false;
	private Vector<RelationQ> relation_q;
	private Vector<RelationR> relation_nlj;
	private AttrType[] qAttrTypes = new AttrType[4];
	private AttrType[] joinResultTypes = new AttrType[4];
	private AttrType[] secondJoinResultTypes = new AttrType[2];
	...
	private NestedLoopsJoins nljCopy = null;
	 ...
	 */
	public JoinsDriverTask2dSolutiona(String QFilePath)
			throws FileNotFoundException {
		boolean status = OK;
		// Q in Task2b.
		...
		qAttrTypes[0] = new AttrType(AttrType.attrInteger);
		qAttrTypes[1] = new AttrType(AttrType.attrInteger);
		qAttrTypes[2] = new AttrType(AttrType.attrInteger);
		qAttrTypes[3] = new AttrType(AttrType.attrInteger);
	...
	public void QueryTask2b() {
		...
		CondExpr[] queryConfiguration = new CondExpr[2];
		queryConfiguration[0] = new CondExpr();
		CondExpr[] queryConfiguration2 = new CondExpr[2];
		queryConfiguration2[0] = new CondExpr();

		// set the query configuration in Task 2b
		setTask2aQueryCondExpr(queryConfiguration,queryConfiguration2);
		...
		// after Rprojection, chose join attributes: R.2, S.3 in the first join
		FldSpec[] joinFields = { new FldSpec(new RelSpec(RelSpec.outer), 3),
		...
		try {
			sjtp = new IESelfJoinWithTwoPredicates(qAttrTypes, 4, null, 10, "Q.in", queryConfiguration,
					queryConfiguration, joinFields, 2);
			sjtp.get_next();
...
//			System.out.println("2#:"+nlj.get_next());
	...
	private void printResult(NestedLoopsJoins nlj2) {
		System.out.println("*** printResult");
		Tuple t = null;
		
		try {
			//t.setHdr((short) 4, rAttrTypes, null);
			int i=0;
			while ((t = nlj2.get_next()) != null) {
				
				t.print(secondJoinResultTypes);
				i++;
	...
	private void setTask2aQueryCondExpr(CondExpr[] expr, CondExpr[] expr2) {
		...
		expr2[0].operand2.symbol = new FldSpec(new RelSpec(RelSpec.innerRel), 4);
	}
